...
namespace DiariesForPractice.Persistence.Extensions
{
	public class TableValuedParameter : SqlMapper.IDynamicParameters
	{
		...
		public TableValuedParameter(string paramName, string typeName)
		{
			_name = paramName;
			_type = typeName;
		}
		...
		public void AddObjectAsRow(object toTable)
		{
			if (_rows == null)
			{
			...
			try
			{
				...
				var values = _properties.Select(property => property.GetValue(toTable)).ToArray();
		...
		public void AddGenericList<T>(IEnumerable<T> list)
		{
